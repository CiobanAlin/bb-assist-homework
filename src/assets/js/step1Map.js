...
google.maps.event.addDomListener(window, "load", function step1Map() {
    ...
    if (html) {
      ...
      link.on('click', function step1Accordion(e) {
          ...
          if($('.accordion-content:eq(1)').hasClass('js-activeAdress')){
            infoWindow.setContent("<h1>The Makeup Academy</h1><p>0752 422 275</p><input type='button' class='button-Step1dot2dot1' value='Select' onclick='step2Map()'/>");
              infoWindow.open(options.map, marker1);
          }

          if($('.accordion-content:eq(2)').hasClass('js-activeAdress')){
            infoWindow.setContent("<h1>Michelle Center</h1><p>0752 556 796</p><input type='button' class='button-Step1dot2dot1' value='Select' onclick='step2Map()'/>");
              infoWindow.open(options.map, marker2);
          }

          if($('.accordion-content:eq(3)').hasClass('js-activeAdress')){
            infoWindow.setContent("<h1>Carmen Style</h1><p>0230 520 888</p><input type='button' class='button-Step1dot2dot1' value='Select' onclick='step2Map()'/>");
  ...
  var marker1 = createMarker({
    ...
    map: map,
  }, "<h1>The Makeup Academy</h1><p>0752 422 275</p><input type='button' class='button-Step1dot2dot1' value='Select' onclick='step2Map()'/>",2);

  var marker2 = createMarker({
    position: new google.maps.LatLng(47.642853,26.248365000000035),
    map: map,
  }, "<h1>Michelle Center</h1><p>0752 556 796</p><input type='button' class='button-Step1dot2dot1' value='Select' onclick='step2Map()'/>",3);

  var marker3 = createMarker({
    position: new google.maps.LatLng(47.6474023,26.247684800000002),
    map: map,
  }, "<h1>Carmen Style</h1><p>0230 520 888</p><input type='button' class='button-Step1dot2dot1' value='Select' onclick='step2Map()'/>",4);

  var marker0 = createMarker({
    position: new google.maps.LatLng(47.6433993,26.248653999999988),
    map: map,
  }, "<h1>Glow Beauty Place</h1><p>0752 566 796</p><input type='button' class='button-Step1dot2dot1' value='Select' onclick='step2Map()'/>",1);
